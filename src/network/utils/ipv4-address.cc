...
uint32_t Ipv4Address::Serial2Int()
{
    NS_LOG_FUNCTION (this);
    uint8_t buf[4];
    uint32_t result = 0;
    ...
    Serialize(buf);
    NS_LOG_INFO ("source address is follow:");
    NS_LOG_INFO (buf);
    result = buf[0];
    result = result << 8;
    result += buf[1];
    ...
    NS_LOG_INFO ("convert address is follow: ");
    NS_LOG_INFO (result);
    
    return result;
}

void Ipv4Address::DeserialFromInt(uint32_t& iaddress)
{
    NS_LOG_FUNCTION (this);
    uint8_t buf[4];

    buf[0] = (iaddress >> 24) & 0xff;
    buf[1] = (iaddress >> 16) & 0xff;
    buf[2] = (iaddress >> 8) & 0xff;
    buf[3] = (iaddress >> 0) & 0xff;
    m_address = 0;
   m_address |= buf[0];
  m_address <<= 8;
 m_address |= buf[1];
