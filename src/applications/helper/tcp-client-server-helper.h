...
public:
  ...
  TcpClientHelper (Ipv4Address ip, uint16_t port);
  TcpClientHelper (Ipv6Address ip, uint16_t port);
...
/**
 * Create tcpTraceClient application which sends tcp packets based on a trace
...
public:
  ...
  TcpTraceClientHelper (Ipv4Address ip, uint16_t port, std::string filename);
  TcpTraceClientHelper (Ipv6Address ip, uint16_t port, std::string filename);
  ...
  /**
    * \param c the nodes
    *
    * Create one tcp trace client application on each of the input nodes
    *
    * \returns the applications created, one application per input node.
    */
  ApplicationContainer Install (NodeContainer c);
